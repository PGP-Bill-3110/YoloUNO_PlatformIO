...
String mainPage()
{
  ...
  <body>
    ...
    <div class="container">
      ...
      <div>
        <button onclick='toggleLED(3)'>🎛️ FSM Mode: <span id="l3">OFF</span></button>
        <button onclick='toggleLED(4)'>🌈 RainBow: <span id="l4">OFF</span></button>
      </div>
      ...
      function toggleLED(id) {
        fetch('/toggle?led='+id)
          ...
          .then(json=>{
            ...
            if(json.rainbow) document.getElementById('l4').innerText=json.rainbow;
          });
      }
...
void handleToggle()
{
  ...
  else if (led == 2)    // đổi thành neo
  {
    // Toggle neo state and cycle color mode
    if (neo_state) {
      // currently ON -> turn OFF
      neo_state = false;
      neoColorMode = 0;
    } else {
      // currently OFF -> turn ON and advance mode
      neo_state = true;
      neoColorMode = (neoColorMode % 3) + 1; // 1..3
    }

    // Apply change to neoStrip safely
    if (neoMutex && xSemaphoreTake(neoMutex, pdMS_TO_TICKS(200)) == pdTRUE) {
      if (neoColorMode == 0) {
        neoStrip.setPixelColor(0, neoStrip.Color(0,0,0));
      } else if (neoColorMode == 1) {
        neoStrip.setPixelColor(0, neoStrip.Color(255,0,0));
      } else if (neoColorMode == 2) {
        neoStrip.setPixelColor(0, neoStrip.Color(0,255,0));
      } else if (neoColorMode == 3) {
